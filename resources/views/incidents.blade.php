@extends('layouts.app')
@section('title','Incident Tickets')
@section('topbar-title','Incident Tickets')
@section('topbar-sub', $tickets->count() . ' ticket' . ($tickets->count() !== 1 ? 's' : '') . ' total')
...
<div class="panel">
    <div class="panel-header">
        <span class="panel-title"><i class="bi bi-exclamation-triangle me-2" style="color:var(--accent)"></i>Incident Tickets</span>
    ...
    <div style="padding:4rem;text-align:center;color:var(--muted)">
        ...
        <div style="font-size:.95rem;font-weight:600">No incident tickets</div>
        <div style="font-size:.82rem;margin-top:.35rem">There are no incident tickets at the moment.</div>
    </div>
    @else
        ...
        <table class="flux-table" id="ticketTable">
            ...
            <tbody>
                ...
                <tr data-status="{{ $t->status }}" data-priority="{{ $t->priority }}" data-search="{{ strtolower($t->ticket_number . ' ' . $t->subject . ' ' . ($t->user->name ?? '') . ' ' . $t->category . ' ' . ($t->type ?? '')) }}">
                    ...
                    <td style="color:var(--muted);font-size:.82rem;white-space:nowrap">{{ $t->assignee ?: 'Unassigned' }}</td>
...
<script>
    ...
    function renderKbaList(kbas) {
        ...
        if (!kbas || kbas.length === 0) { el.innerHTML = '<div style="font-size:.78rem;color:var(--muted);padding:.35rem 0">No KBAs linked yet.</div>'; return; }
        ...
        el.innerHTML = kbas.map(k => {
            ...
            return `<div style="display:inline-flex;align-items:center;gap:.4rem;background:rgba(99,102,241,.1);border:1px solid rgba(99,102,241,.2);border-radius:.5rem;padding:.25rem .6rem;margin:0 .35rem .35rem 0;font-size:.78rem;color:#818cf8"><i class="bi bi-journal-text"></i><span style="font-weight:600">${escHtml(k.kba_number||'#KBA-'+String(k.id).padStart(4,'0'))}</span><span style="color:var(--muted)">·</span><span>${escHtml(k.title)}</span>${removeBtn}</div>`;
        }).join('');
    }
    ...
    function filterKbaDropdown(q) {
        ...
        const matches = _allKbas.filter(k=>!linked.includes(k.id)&&(!term||k.title.toLowerCase().includes(term)||k.kba_number.toLowerCase().includes(term)||(k.category||'').toLowerCase().includes(term))).slice(0,20);
        const dd = document.getElementById('vm-kba-dropdown');
        if (!matches.length) { dd.innerHTML='<div style="padding:.5rem .85rem;font-size:.78rem;color:var(--muted)">No matching KBAs found.</div>'; }
        else { dd.innerHTML=matches.map(k=>`<div onclick="confirmAttachKba(${k.id})" style="padding:.45rem .85rem;cursor:pointer;font-size:.8rem;transition:background .1s;display:flex;align-items:center;gap:.5rem" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background=''"><i class="bi bi-journal-text" style="color:#818cf8;flex-shrink:0"></i><span style="color:#818cf8;font-weight:600;flex-shrink:0">${escHtml(k.kba_number||'#KBA-'+String(k.id).padStart(4,'0'))}</span><span style="color:var(--text);flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escHtml(k.title)}</span><span style="color:var(--muted);font-size:.72rem;flex-shrink:0">${escHtml(k.category||'')}</span></div>`).join(''); }
        dd.style.display='';
    }
    ...
    document.addEventListener('click',function(e){ const s=document.getElementById('vm-kba-search'),dd=document.getElementById('vm-kba-dropdown'); if(!dd||dd.style.display==='none') return; if(s&&(s.contains(e.target)||dd.contains(e.target))) return; dd.style.display='none'; });
    let _pendingKbaId=null;
    function confirmAttachKba(articleId) {
        hideKbaDropdown(); const kba=_allKbas.find(k=>k.id===articleId); _pendingKbaId=articleId;
        document.getElementById('kba-confirm-num').textContent=kba?(kba.kba_number||'#KBA-'+String(kba.id).padStart(4,'0')):'';
        document.getElementById('kba-confirm-title').textContent=kba?kba.title:'';
        const overlay=document.getElementById('kbaConfirmBackdrop'),box=document.getElementById('kbaAttachBox');
        overlay.style.display='flex'; requestAnimationFrame(()=>requestAnimationFrame(()=>{ box.style.transform='scale(1) translateY(0)'; box.style.opacity='1'; }));
    }
    function closeKbaAttachModal() {
        const overlay=document.getElementById('kbaConfirmBackdrop'),box=document.getElementById('kbaAttachBox');
        box.style.transform='scale(.94) translateY(8px)'; box.style.opacity='0';
        setTimeout(()=>{ overlay.style.display='none'; const btn=document.getElementById('kbaAttachOk'),icon=document.getElementById('kbaAttachOkIcon'),spinner=document.getElementById('kbaAttachOkSpinner'),label=document.getElementById('kbaAttachOkLabel'); if(btn){btn.disabled=false;icon.style.display='';spinner.style.display='none';label.textContent='Yes, Attach KBA';} },200);
        _pendingKbaId=null; document.getElementById('vm-kba-search').value='';
    }
    function doAttachKba() {
        if(!_pendingKbaId) return; const articleId=_pendingKbaId;
        const btn=document.getElementById('kbaAttachOk'),icon=document.getElementById('kbaAttachOkIcon'),spinner=document.getElementById('kbaAttachOkSpinner'),label=document.getElementById('kbaAttachOkLabel');
        if(btn){btn.disabled=true;icon.style.display='none';spinner.style.display='inline-block';label.textContent='Attaching…';}
        fetch(`/tickets/${_currentTicketId}/kba/${articleId}`,{method:'POST',headers:{'X-CSRF-TOKEN':_csrfToken,'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(data=>{ closeKbaAttachModal(); if(data.ok){if(!_kbaCache[_currentTicketId])_kbaCache[_currentTicketId]=[];_kbaCache[_currentTicketId].push(data.article);renderKbaList(_kbaCache[_currentTicketId]);} }).catch(()=>{ closeKbaAttachModal(); });
    }
    function detachKba(articleId) {
        ...
        showKbaConfirm(label,function(){ fetch(`/tickets/${_currentTicketId}/kba/${articleId}`,{method:'DELETE',headers:{'X-CSRF-TOKEN':_csrfToken,'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}}).then(r=>r.json()).then(data=>{ if(data.ok){_kbaCache[_currentTicketId]=(_kbaCache[_currentTicketId]||[]).filter(k=>k.id!==articleId);renderKbaList(_kbaCache[_currentTicketId]);} }).catch(()=>{}); });
    }
    function renderNoteHtml(n) {
        const isRoute=n.type==='route_event',isRejection=n.type==='rejection',isSystem=n.type==='system',dateStr=new Date(n.created_at).toLocaleString('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'}),rawBody=(n.content||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/^&gt; (.+)$/gm,'<div style="border-left:3px solid rgba(248,113,113,.3);padding-left:.6rem;color:var(--muted);margin-top:.3rem;font-size:.8rem">$1</div>');
        if(isRejection){return `<div style="display:flex;gap:.7rem;margin-bottom:.85rem"><div style="width:28px;height:28px;min-width:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:700;color:#1a1a2e;background:linear-gradient(135deg,#f59e0b,#d97706)">SYS</div><div style="flex:1;min-width:0"><div style="display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;margin-bottom:.3rem"><span style="font-size:.8rem;font-weight:600;color:#fbbf24">System</span><span style="font-size:.63rem;font-weight:700;text-transform:uppercase;background:rgba(248,113,113,.15);color:#f87171;padding:.1rem .45rem;border-radius:9999px"><i class="bi bi-x-circle-fill"></i> Rejected</span><span style="font-size:.7rem;color:var(--muted)">${dateStr}</span></div><div style="font-size:.83rem;color:var(--text);line-height:1.55;background:rgba(248,113,113,.07);border:1px solid rgba(248,113,113,.25);border-radius:.5rem;padding:.55rem .8rem">${rawBody}</div></div></div>`;}
        if(isSystem){return `<div style="display:flex;gap:.7rem;margin-bottom:.85rem"><div style="width:28px;height:28px;min-width:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.6rem;font-weight:700;color:#1a1a2e;background:linear-gradient(135deg,#f59e0b,#d97706)">SYS</div><div style="flex:1;min-width:0"><div style="display:flex;align-items:center;gap:.45rem;flex-wrap:wrap;margin-bottom:.3rem"><span style="font-size:.8rem;font-weight:600;color:#fbbf24">System</span><span style="font-size:.63rem;font-weight:700;text-transform:uppercase;background:rgba(251,191,36,.12);color:#fbbf24;padding:.1rem .45rem;border-radius:9999px"><i class="bi bi-info-circle-fill"></i> System</span><span style="font-size:.7rem;color:var(--muted)">${dateStr}</span></div><div style="font-size:.83rem;color:var(--text);line-height:1.55;background:rgba(251,191,36,.06);border:1px solid rgba(251,191,36,.18);border-radius:.5rem;padding:.55rem .8rem">${rawBody}</div></div></div>`;}
        const initial=(n.user?.name||'?').charAt(0).toUpperCase(),avatarBg=isRoute?'linear-gradient(135deg,#2563eb,#4f46e5)':'linear-gradient(135deg,#4f46e5,#7c3aed)',chip=isRoute?`<span style="font-size:.63rem;font-weight:700;text-transform:uppercase;background:rgba(59,130,246,.15);color:#60a5fa;padding:.1rem .45rem;border-radius:9999px"><i class="bi bi-arrow-left-right"></i> Routed</span>`:`<span style="font-size:.63rem;font-weight:700;text-transform:uppercase;background:rgba(99,102,241,.15);color:#818cf8;padding:.1rem .45rem;border-radius:9999px"><i class="bi bi-chat-left-text"></i> Note</span>`,attachHtml=n.attachment?`<div style="margin-top:.4rem"><a href="/storage/${n.attachment}" target="_blank" style="font-size:.75rem;color:#818cf8;display:inline-flex;align-items:center;gap:.3rem"><i class="bi bi-paperclip"></i> View attachment</a></div>`:'';
    ...
    function updateNoteFileLabel(input) {
        ...
        if(input.files.length){if(input.files[0].size>5*1024*1024){input.value='';lbl.textContent='Attach a file (max 5 MB)';const e=document.getElementById('vm-note-error');e.textContent='File exceeds 5 MB limit.';e.style.display='';return;}lbl.textContent='📎 '+input.files[0].name;}else{lbl.textContent='Attach a file (max 5 MB)';}
    }
    async function submitNote() {
        const content=document.getElementById('vm-note-input').value.trim(),fileInput=document.getElementById('vm-note-file'),errEl=document.getElementById('vm-note-error');
        errEl.style.display='none';
        if(!content){errEl.textContent='Note cannot be empty.';errEl.style.display='';return;}
        const fd=new FormData();fd.append('_token',_csrfToken);fd.append('content',content);if(fileInput.files.length)fd.append('attachment',fileInput.files[0]);
        const btn=document.getElementById('vm-note-submit');btn.disabled=true;btn.innerHTML='<span style="display:inline-block;width:12px;height:12px;border:2px solid rgba(255,255,255,.4);border-top-color:white;border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle;margin-right:.3rem"></span> Adding…';
        try{const r=await fetch('/tickets/'+_currentTicketId+'/notes',{method:'POST',headers:{'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':_csrfToken},body:fd});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.message||'Server error');if(!_notesCache[_currentTicketId])_notesCache[_currentTicketId]=[];_notesCache[_currentTicketId].push(data.note);const tl=document.getElementById('vm-notes-timeline');const empty=tl.querySelector('[style*="font-style:italic"]');if(empty)tl.innerHTML='';tl.insertAdjacentHTML('beforeend',renderNoteHtml(data.note));document.getElementById('vm-note-input').value='';fileInput.value='';document.getElementById('vm-note-file-label').textContent='Attach a file (max 5 MB)';}catch(e){errEl.textContent=e.message||'Failed to add note.';errEl.style.display='';}finally{btn.disabled=false;btn.innerHTML='<i class="bi bi-chat-left-text-fill"></i> Add Note';}
    }
    document.addEventListener('keydown',e=>{if(e.key==='Escape')['viewModal','routeModal'].forEach(closeModal);});

    const _FKEY='flux_filter_incidents';
    let _currentStatus='all',_currentPriority='all';
    function _saveFilter(){try{localStorage.setItem(_FKEY,JSON.stringify({status:_currentStatus,priority:_currentPriority}));}catch(e){}}
    function filterStatus(s,btn){_currentStatus=s;document.querySelectorAll('#statusFilters .filter-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');_saveFilter();applyFilters();}
    function filterPriority(p,btn){_currentPriority=p;document.querySelectorAll('#priorityFilters .filter-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');_saveFilter();applyFilters();}
    function applyFilters(){const q=(document.getElementById('searchInput')?.value||'').toLowerCase();document.querySelectorAll('#ticketTable tbody tr').forEach(row=>{const matchStatus=_currentStatus==='all'||row.dataset.status===_currentStatus,matchPriority=_currentPriority==='all'||row.dataset.priority===_currentPriority,matchSearch=!q||(row.dataset.search||'').includes(q);row.style.display=(matchStatus&&matchPriority&&matchSearch)?'':'none';});updateTableScroll();}
    function updateTableScroll(){const wrap=document.getElementById('tableWrap');if(!wrap)return;const visible=[...wrap.querySelectorAll('#ticketTable tbody tr')].filter(r=>r.style.display!=='none').length;if(visible>9){wrap.style.maxHeight='540px';wrap.style.overflowY='auto';}else{wrap.style.maxHeight='';wrap.style.overflowY='hidden';}}
    updateTableScroll();
    (function(){try{const saved=JSON.parse(localStorage.getItem(_FKEY));if(!saved)return;if(saved.status&&saved.status!=='all'){const b=document.querySelector('#statusFilters [data-status="'+saved.status+'"]');if(b)filterStatus(saved.status,b);}if(saved.priority&&saved.priority!=='all'){const b=document.querySelector('#priorityFilters [data-priority="'+saved.priority+'"]');if(b)filterPriority(saved.priority,b);}}catch(e){}})();
    ...
    function openView(t,requesterName){
        ...
        const sMap={open:'s-open',progress:'s-progress',resolved:'s-resolved',closed:'s-closed'},sLabel={open:'Open',progress:'In Progress',resolved:'Resolved',closed:'Closed'},sb=document.getElementById('vm-status-badge');
        sb.className='badge-status '+(sMap[t.status]||'s-open');sb.textContent=sLabel[t.status]||t.status;
        ...
        const isAssignee=t.assignee&&t.assignee===currentUser,isDone=['resolved','closed'].includes(t.status),editSection=document.getElementById('vm-edit-section'),saveBtn=document.getElementById('vm-save-btn'),routeBtn=document.getElementById('vm-route-btn');
        editSection.style.display=isAssignee?'':'none';saveBtn.style.display=isAssignee?'':'none';routeBtn.style.display=(isAssignee&&!isDone)?'':'none';
        if(isAssignee){document.getElementById('editTicketForm').action='/tickets/'+t.id;document.getElementById('vm-status-sel').value=t.status;document.getElementById('vm-assignee-inp').value=t.assignee||'';document.getElementById('vm-resolution-inp').value=t.resolution||'';const prioritySel=document.getElementById('vm-priority-sel'),priorityHidden=document.getElementById('vm-priority-hidden');prioritySel.value=t.priority;priorityHidden.value=t.priority;prioritySel.disabled=isDone;prioritySel.style.opacity=isDone?'.55':'';prioritySel.style.cursor=isDone?'not-allowed':'';}
        const slaDue=document.getElementById('vm-sla-due'),slaBadge=document.getElementById('vm-sla-badge'),slaBarWrap=document.getElementById('vm-sla-bar-wrap'),slaBar=document.getElementById('vm-sla-bar'),slaPctLabel=document.getElementById('vm-sla-pct-label'),slaTimeLabel=document.getElementById('vm-sla-time-label');
        if(t.sla_due_at){const due=new Date(t.sla_due_at),created=new Date(t.created_at),now=new Date(),isDone2=['resolved','closed'].includes(t.status),compareAt=isDone2&&t.resolved_at?new Date(t.resolved_at):now;let ss;if(compareAt>due)ss='breached';else if(isDone2)ss='met';else{const total=due-created,rem=due-now;ss=(rem/total)<=0.25?'warning':'ok';}slaDue.textContent=new Date(t.sla_due_at).toLocaleString('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'});slaBadge.innerHTML=`<span class="sla-badge sla-${ss}"><span class="sla-dot"></span>${slaLabels[ss]}</span>`;if(!isDone2){slaBarWrap.style.display='';const totalSec=(due-created)/1000,usedSec=(now-created)/1000,pct=Math.min(100,Math.round((usedSec/totalSec)*100)),remMs=due-now,remHrs=Math.round(remMs/3600000),remDays=Math.floor(remHrs/24),remLabel=remMs<0?`Overdue by ${Math.abs(remDays)}d ${Math.abs(remHrs%24)}h`:remDays>0?`${remDays}d ${remHrs%24}h remaining`:`${remHrs}h remaining`;slaBar.style.width=pct+'%';slaBar.style.background=slaColors[ss];slaPctLabel.textContent=pct+'% elapsed';slaTimeLabel.textContent=remLabel;}else{slaBarWrap.style.display='none';}}else{slaDue.textContent='—';slaBadge.innerHTML='<span style="color:var(--muted)">—</span>';slaBarWrap.style.display='none';}
        ...
        _currentTicketId=t.id;_currentTicketStatus=t.status;
        if(_notesCache[t.id]===undefined)_notesCache[t.id]=[...(t.notes||[])];
        renderTimeline(_notesCache[t.id]);
        if(_kbaCache[t.id]===undefined)_kbaCache[t.id]=[...(t.knowledge_articles||[])];
        renderKbaList(_kbaCache[t.id]);
        document.getElementById('vm-kba-search').value='';hideKbaDropdown();
        ...
        document.getElementById('vm-note-input').value='';document.getElementById('vm-note-file').value='';document.getElementById('vm-note-file-label').textContent='Attach a file (max 5 MB)';document.getElementById('vm-note-error').style.display='none';
        const _vmBody=document.querySelector('#viewModal .flux-modal-body');if(_vmBody){_vmBody.style.opacity='0';_vmBody.style.transition='';}
        openModal('viewModal');requestAnimationFrame(()=>requestAnimationFrame(()=>{if(_vmBody){_vmBody.style.transition='opacity .3s ease';_vmBody.style.opacity='1';}}));
    }
    function openRouteFromView(){const btn=document.getElementById('vm-route-btn');document.getElementById('route-sub').textContent=btn.dataset.ticketNum+' — Forward to another department';document.getElementById('routeForm').action='/tickets/'+btn.dataset.ticketId+'/route';document.getElementById('route-dept').value='';const ps=document.getElementById('route-person');ps.innerHTML='<option value="" disabled selected>Select department first…</option>';ps.disabled=true;closeModal('viewModal');openModal('routeModal');}
    function loadRouteDeptUsers(dept){const sel=document.getElementById('route-person'),du=window.deptUsers||{};sel.innerHTML='<option value="" disabled selected>Select person…</option>';(du[dept]||[]).forEach(name=>{const o=document.createElement('option');o.value=name;o.textContent=name;sel.appendChild(o);});sel.disabled=!(du[dept]||[]).length;}

    (function(){
        const overlayHtml=`<div id="kbaConfirmModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.55);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);align-items:center;justify-content:center"><div id="kbaConfirmBox" style="background:var(--surface,#1e1e2e);border:1px solid var(--border,rgba(255,255,255,.08));border-radius:1rem;padding:1.75rem 1.75rem 1.4rem;max-width:420px;width:90%;box-shadow:0 25px 60px rgba(0,0,0,.5);transform:scale(.94) translateY(8px);opacity:0;transition:transform .22s cubic-bezier(.34,1.56,.64,1),opacity .18s ease"><div style="display:flex;align-items:flex-start;gap:.9rem;margin-bottom:1.1rem"><div style="width:38px;height:38px;min-width:38px;border-radius:.65rem;background:rgba(248,113,113,.12);border:1px solid rgba(248,113,113,.25);display:flex;align-items:center;justify-content:center"><i class="bi bi-journal-x" style="color:#f87171;font-size:1rem"></i></div><div><div style="font-size:.95rem;font-weight:700;color:var(--text,#e2e8f0);margin-bottom:.3rem">Remove KBA Link</div><div id="kbaConfirmMsg" style="font-size:.83rem;color:var(--muted,#94a3b8);line-height:1.55"></div></div></div><div style="background:rgba(251,191,36,.06);border:1px solid rgba(251,191,36,.18);border-radius:.55rem;padding:.55rem .75rem;display:flex;align-items:center;gap:.5rem;margin-bottom:1.3rem"><i class="bi bi-info-circle" style="color:#fbbf24;font-size:.8rem;flex-shrink:0"></i><span style="font-size:.78rem;color:#fbbf24">You can re-attach this KBA later if needed.</span></div><div style="display:flex;justify-content:flex-end;gap:.6rem"><button id="kbaConfirmCancel" style="padding:.45rem 1.1rem;border-radius:.55rem;border:1px solid var(--border,rgba(255,255,255,.1));background:var(--surface2,rgba(255,255,255,.05));color:var(--text,#e2e8f0);font-size:.83rem;font-weight:600;cursor:pointer;transition:background .15s">Cancel</button><button id="kbaConfirmOk" style="padding:.45rem 1.2rem;border-radius:.55rem;border:none;background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;font-size:.83rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:.4rem;transition:opacity .15s"><i class="bi bi-trash3"></i> Remove</button></div></div></div>`;
        document.body.insertAdjacentHTML('beforeend',overlayHtml);
        document.body.insertAdjacentHTML('beforeend',`<div id="kbaConfirmBackdrop" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.55);backdrop-filter:blur(4px);align-items:center;justify-content:center"><div id="kbaAttachBox" style="background:var(--surface,#1e1e2e);border:1px solid var(--border,rgba(255,255,255,.08));border-radius:1rem;padding:1.75rem 1.75rem 1.4rem;max-width:420px;width:90%;box-shadow:0 25px 60px rgba(0,0,0,.5);transform:scale(.94) translateY(8px);opacity:0;transition:transform .22s cubic-bezier(.34,1.56,.64,1),opacity .18s ease"><div style="display:flex;align-items:flex-start;gap:.9rem;margin-bottom:1.1rem"><div style="width:38px;height:38px;min-width:38px;border-radius:.65rem;background:rgba(99,102,241,.15);border:1px solid rgba(99,102,241,.3);display:flex;align-items:center;justify-content:center"><i class="bi bi-journal-plus" style="color:#818cf8;font-size:1rem"></i></div><div style="min-width:0;flex:1"><div style="font-size:.95rem;font-weight:700;color:var(--text,#e2e8f0);margin-bottom:.3rem">Attach Knowledge Article?</div><div style="font-size:.83rem;color:var(--muted,#94a3b8);line-height:1.55">You are about to link <span id="kba-confirm-num" style="color:#818cf8;font-weight:700"></span> <span id="kba-confirm-title" style="color:var(--text,#e2e8f0)"></span> to this ticket.</div></div></div><div style="display:flex;justify-content:flex-end;gap:.6rem"><button id="kbaAttachCancel" style="padding:.45rem 1.1rem;border-radius:.55rem;border:1px solid var(--border,rgba(255,255,255,.1));background:var(--surface2,rgba(255,255,255,.05));color:var(--text,#e2e8f0);font-size:.83rem;font-weight:600;cursor:pointer;transition:background .15s">Cancel</button><button id="kbaAttachOk" onclick="doAttachKba()" style="padding:.45rem 1.2rem;border-radius:.55rem;border:none;background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;font-size:.83rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:.4rem;box-shadow:0 3px 12px rgba(99,102,241,.35);transition:opacity .15s"><i class="bi bi-journal-plus" id="kbaAttachOkIcon"></i><span style="display:none;width:13px;height:13px;border:2px solid rgba(255,255,255,.35);border-top-color:white;border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle" id="kbaAttachOkSpinner"></span><span id="kbaAttachOkLabel">Yes, Attach KBA</span></button></div></div></div>`);
        let _kbaConfirmCb=null;
        const overlay=document.getElementById('kbaConfirmModal'),box=document.getElementById('kbaConfirmBox'),msg=document.getElementById('kbaConfirmMsg'),okBtn=document.getElementById('kbaConfirmOk'),cancelBtn=document.getElementById('kbaConfirmCancel');
        function openKbaConfirm(){overlay.style.display='flex';requestAnimationFrame(()=>requestAnimationFrame(()=>{box.style.transform='scale(1) translateY(0)';box.style.opacity='1';}));}
        function resetOkBtn(){okBtn.disabled=false;okBtn.innerHTML='<i class="bi bi-trash3"></i> Remove';okBtn.style.opacity='1';}
        function closeKbaConfirm(){box.style.transform='scale(.94) translateY(8px)';box.style.opacity='0';setTimeout(()=>{overlay.style.display='none';resetOkBtn();},200);_kbaConfirmCb=null;}
        window.showKbaConfirm=function(label,onConfirm){msg.innerHTML=`Remove <strong style="color:var(--text,#e2e8f0)">"${label}"</strong> from this ticket?`;_kbaConfirmCb=onConfirm;resetOkBtn();openKbaConfirm();};
        okBtn.addEventListener('click',function(){const cb=_kbaConfirmCb;okBtn.disabled=true;okBtn.innerHTML='<span style="display:inline-block;width:13px;height:13px;border:2px solid rgba(255,255,255,.35);border-top-color:white;border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle;margin-right:.3rem"></span>Removing…';closeKbaConfirm();if(typeof cb==='function')cb();});
        cancelBtn.addEventListener('click',closeKbaConfirm);overlay.addEventListener('click',function(e){if(e.target===overlay)closeKbaConfirm();});document.addEventListener('keydown',function(e){if(e.key==='Escape'&&overlay.style.display==='flex')closeKbaConfirm();});
        okBtn.addEventListener('mouseover',()=>{if(!okBtn.disabled)okBtn.style.opacity='.85';});okBtn.addEventListener('mouseout',()=>{if(!okBtn.disabled)okBtn.style.opacity='1';});
        cancelBtn.addEventListener('mouseover',()=>cancelBtn.style.background='var(--surface,rgba(255,255,255,.08))');cancelBtn.addEventListener('mouseout',()=>cancelBtn.style.background='var(--surface2,rgba(255,255,255,.05))');
    })();
</script>
@endpush
...
<script>
(function(){
    const overlay=document.getElementById('kbaConfirmBackdrop'),cancelBtn=document.getElementById('kbaAttachCancel');
    ...
    document.addEventListener('keydown',function(e){if(e.key==='Escape'&&overlay.style.display==='flex')closeKbaAttachModal();});
})();
</script>
@endpush
